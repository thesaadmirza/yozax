...
$(function () {
    ...
    $(document).on('click', 'a.ad-renderer.ad', function(event) {
      ...
      if (ad_id) {
        ...
        self.removeClass('ad');
        $.post(YX_Ajax_Requests_File(), {f:'ads',s:'rc',id:ad_id});
      }
    });
    ...
    $(document).on('submit','form.share-e-form',function(event){
      ...
      YX_progressIconLoader(self.find('button')); 
      $.ajax({
        url: YX_Ajax_Requests_File() + "?f=share",
        type: 'POST',
        dataType: 'json',
        data: data,
      })
...
function YX_progressIconLoader(container_elem) {
...
function YX_ShowNavList(type) {
  $('.news-header-dropdown').show();
  $.get(YX_Ajax_Requests_File(), {f: 'get_menu_list', type:type}, function (data) {
...
function YX_AddEntry(type) {
  YX_progressIconLoader($('#add-entry-button'));
  $.get(YX_Ajax_Requests_File(), {f: 'add_entry', type:type}, function (data) {
...
function YX_FetchVideo(id) {
  ...
  YX_progressIconLoader(video_container.find('button.btn-blue'));
  var hash_id = $('.hash_id').val();
  $.post(YX_Ajax_Requests_File() + '?f=fetch_video', {video_url:video_url, id:id, hash_id:hash_id}, function (data) {
    if (data.status == 200) {
...
function YX_FetchImage(id, set_as_lang) {
  var image_container = $('#entry-' + id);
  var image_url = image_container.find('#entry-image-link').val();
  var hash_id   = $('.hash_id').val();
  YX_progressIconLoader(image_container.find('button'));
  $.post(YX_Ajax_Requests_File() + '?f=fetch_image', {image_url:image_url, id:id, hash_id:hash_id}, function (data) {
    if (data.status == 200) {
        ...
        image_container.find('.img-content').find('.img-set-as').html("<span class='btn btn-main yx_btn' onclick='YX_MakePreviewImage(" + id + ")'><i class='fa fa-picture-o'></i> " + set_as_lang + "</span>");
        if ($('#post-preview-image').find('.content').find('img').length == 0) {
           $('#post-preview-image').find('.content').html('<div class="post-image"><img src="' + data.img + '"/></div>');
           $('#post-preview-image-input').val(data.img);
        }
...
function YX_FetchQuestionImage(id) {
  var image_container = $('#entry-' + id);
  var image_url = image_container.find('#entry-image-link').val();
  var hash_id   = $('.hash_id').val();
  YX_progressIconLoader(image_container.find('button'));
...
function YX_UploadImage(id, set_as_lang) {
  var image_container = $('#entry-' + id);
  ...
  $.ajax({
        type: "POST",
        url: YX_Ajax_Requests_File() + '?f=upload_image&id=' + id + '&hash_id=' + hash_id,
        data: data,
        processData: false,
        contentType: false,
...
function YX_UploadResultCover(id) {
  var image_container = $('#entry-' + id);
  var fetched_image = image_container.find('#fetched-image');
  var data = new FormData();
  data.append('image', $('#entry-image-file-' + id).prop('files')[0]);
...
function YX_GetTweet(id) {
  ...
  YX_progressIconLoader(tweet_container.find('button'));
  $.post(YX_Ajax_Requests_File() + '?f=fetch_tweet', {id:id, tweet_url:tweet_url, hash_id:hash_id}, function (data) {
    if (data.status == 200) {
...
function YX_GetInstagramPost(id) {
  ...
  YX_progressIconLoader(instagram_container.find('button'));
  $.post(YX_Ajax_Requests_File() + '?f=fetch_instagram', {id:id, instagram_url:instagram_url, hash_id:hash_id}, function (data) {
    if (data.status == 200) {
...
function YX_GetSoundCloudPost(id) {
  ...
  YX_progressIconLoader(soundcloud_container.find('button'));
  $.post(YX_Ajax_Requests_File() + '?f=fetch_soundcloud', {id:id, soundcloud_url:soundcloud_url, hash_id:hash_id}, function (data) {
    if (data.status == 200) {
...
function YX_GetFacebookPost(id) {
  ...
  var hash_id = $('.hash_id').val();
  YX_progressIconLoader(facebook_container.find('button'));
  facebook_container.find('.error-con').empty();
  $.post(YX_Ajax_Requests_File() + '?f=fetch_facebook', {id:id, facebook_url:facebook_url, hash_id:hash_id}, function (data) {
    if (data.status == 200) {
...
function YX_UploadPreviewImage() {
  var data = new FormData();
...
function YX_MakePreviewImage(id) {
    var image_container = $('#entry-' + id);
...
function YX_RemoveEntryByID(id) {
  var box = $('#entry-' + id).slideUp(200, function () {
     $('#entry-' + id).remove();
     $.get(YX_Ajax_Requests_File(), {f: 'delete_entry', id:id});
  });
}
function YX_OpenFetchImageInput(id) {
...
function YX_AddAnswer(id, answer_key) {
...
function YX_Vote(entry_id, id) {
  ...
  $.get(YX_Ajax_Requests_File(), {f: 'vote', id:id, entry_id:entry_id}, function (data) {
      if (data.status == 200) {
...
function YX_VoteImg(entry_id, id) {
  var vote_container = $('#answer-' + id);
  var attr = $('#e-' + entry_id).find('.entry-options').attr('data-clicked');
  if (typeof attr !== typeof undefined && attr !== false) {
      alert('You have already voted this poll.');
...
function YX_ShareLink(href, e, id, type) {
  e.preventDefault();
  if (href != 'none') {
  ...
  $.get(YX_Ajax_Requests_File(), {f:'share_link', id:id, type:type}, function (data) {
...
function YX_DeletePost(id, type) {
  ...
  $.get(YX_Ajax_Requests_File(), {f:'delete_post', id:id, type:type}, function (data) {
    if (data.status == 200) {
...
function YX_LoadMore(where, type, id, user_id, button, c_id, keyword, page_in) {
  ...
  $.get(YX_Ajax_Requests_File(), {f:'load_more', id:id, type:type, where:where, user_id:user_id, c_id:c_id, keyword:keyword_, page:page}, function (data) {
      if (data) {
        $('#' + where).append(data);
      } else {
...
function YX_DeleteUserFromAdmin(id) {
  if (!id) { return false; }
  ...
  $.get(YX_Ajax_Requests_File(), {f:'admincp', s:'delete_user', id:id});
}

function YX_ActiveAction(type, id, activation, layout) {
  if (!id) { return false; }
  ...
  $.get(YX_Ajax_Requests_File(), {f:'admincp', s:'activation', id:id, type:type, activation:activation}, function (data) {

  });
}

function YX_ActiveUserAction(id, activation, layout) {
  if (!id) { return false; }
  if (!activation) { return false; }
  if (activation == 'activate') {
    $('tr[data-id="' + id + '"]').find('.activation-text').removeClass('active-0').addClass('active-1').text('Active');
  ...
  $.get(YX_Ajax_Requests_File(), {f:'admincp', s:'user_activation', id:id, activation:activation}, function (data) {

  });
}

function YX_UploadOptImg(id = false,self = false){
  ...
  $.ajax({
        type: "POST",
        url: YX_Ajax_Requests_File() + '?f=upload_opt_img&hash_id=' + hash_id,
        data: data,
        processData: false,
        contentType: false,
        success: function (data) {
...
function YX_UploadQuestionImg(id = false,self = false){
  if (!id || !self) { return false;}
  ...
  $.ajax({
        type: "POST",
        url: YX_Ajax_Requests_File() + '?f=quiz&s=upload&hash_id=' + hash_id,
        data: data,
        processData: false,
        contentType: false,
        success: function (data) {
...
function YX_RegisterReply(text = false,comm_id = false,event = false,post_id = false,page = false){
  ...
  if (event.keyCode == 13 && event.shiftKey == 0) {
    if (!YX_IsLoggedIn()) {
      ...
      YX_progressIconLoader($("#comm-reply-"+comm_id).find('small'));
      $.ajax({
        url: YX_Ajax_Requests_File() + '?f=comments&s=reply',
        type: 'POST',
        dataType: 'json',
      ...
      .done(function(data) {
        if (data.status == 200) {
          $("#yx-comment-item-replies-list-" + comm_id).append(data.html)
...
function YX_DeleteComment(comm_id = false,page = false){
  if (!YX_IsLoggedIn()) {
    try{
  ...
  if (comm_id && page) {
    $.ajax({
      url: YX_Ajax_Requests_File() + '?f=comments&s=delete',
      type: 'POST',
      dataType: 'json',
...
function YX_DeleteReply(reply_id = false){
  if (reply_id) {
    $.ajax({
      url: YX_Ajax_Requests_File() + '?f=comments&s=delete-reply',
      type: 'POST',
      dataType: 'json',
      data: {id:reply_id},
    })
...
function YX_StartBar() {
    $('#yx-loading-bar').height("3").show().animate({width:20 + 80 * Math.random() + "%"}, 500);
}

function YX_FinishBar() {
    $('#yx-loading-bar').animate({width:"100%"}, 500).delay(300).fadeOut(300, function() {
       $(this).width("0").height("0");
    });
}

function YX_ArrayMode(arr) {
    var uniqs = {};
...
function YX_GetQuizResult(ans,post,index,entry,self){
  ...
  if($("[data-question=0]").length > 0) {
    ...
    var result_index = YX_ArrayMode(window.quiz_results);
...
function YX_HideAnnouncement(id){
  if (!id) { return false;}
  $.ajax({
    url: YX_Ajax_Requests_File(),
    type: 'GET',
  ...
  .done(function(data) {
    if (data.status == 200) {
      $("#yx-announcement-"+id).slideUp('fast',function(){
        $(this).remove();
      })
    }
  })
...
$(document).on('click','#follow',function(event){
  ...
  $.ajax({
    url: YX_Ajax_Requests_File() + "?type=follow",
    type: 'POST',
    dataType: 'json',
    data: data,
  })
...
$(document).on('click','.Tochat',function(){
  LordChatheader($(this).attr('data-id'));
});


$(document).on('change','#ChatHeader',function(){
  LordChat($(this).attr('data-id'));
});



$(document).on('click','#addmsg',function(){
    addmsg($('#ChatHeader').attr('data-id'),$('#msg').val());
});


$(document).on('click','.Tochat',function(){
      $('.Tochat').removeClass('active-to-chat');
      $(this).addClass('active-to-chat');
});


function LordFollowers(){
            ...
            var  List = function () {
                ...
                $.ajax({
                    async: false,
                    type: 'post',
                    url: YX_Ajax_Requests_File() + "?type=followlist",
                    data:{},
                    global: false,
                    success: function(data){
                        tmp = data;
            ...
            if(data.status){
              $('#ListOfFollowers').html('').html(data.content);
            }
...
function LordChatheader(val){
    ...
    var  header = function () {
      ...
      $.ajax({
          async: false,
          type: 'post',
          url: YX_Ajax_Requests_File() + "?type=chatheader",
          data:{
            id:val
          },
          global: false,
    ...
    if(header.status){
      $('#ChatHeader').html('').html(header.content);
      $('#ChatHeader').attr('data-id',val);
      $('#ChatHeader').trigger('change');
    }
...
function LordChat(val){
    ...
    var  chatList = function () {
      ...
      $.ajax({
          async: false,
          type: 'post',
          url: YX_Ajax_Requests_File() + "?type=chatlist",
          data:{
            id:val
          },
          global: false,
    ...
    if(chatList.status){
        $('#chat').html('').html(chatList.content);
...
function addmsg(id,msg){
    ...
    var  data = function () {
      ...
      $.ajax({
          async: false,
          type: 'post',
          url: YX_Ajax_Requests_File() + "?type=addmsg",
          data:{
            id:id,
            msg:msg
          },
...
function setSelect(val){
  $(".Tochat[data-id='" + val +"']").trigger('click');
}
