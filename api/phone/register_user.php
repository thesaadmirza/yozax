...
if ($type == 'user_registration') {
    ...
    } else if (YX_UserExists($_POST['username'])) {
    	$json_error_data['errors']  = array(
            'error_id'              => '3',
        ...
        if ($register === true) {
            if ($activate == 1) {
                $json_success_data  = array(
                    ...
                    'success_type'  => 'registered',
                    'session_id'    => 0,
                    'cookie'        => YX_CreateLoginSession(YX_UserIdForLogin($username)),
                    'user_id'       => 0
                );
                $s       = $_POST['s'];
                $s_md5   = md5($_POST['s']);
                ...
                $add_session = mysqli_query($sqlConnect, "INSERT INTO " . T_APP_SESSIONS . " (`user_id`, `session_id`, `platform`, `platform_info`,`time`) VALUES ('{$user_id}', '{$s_md5}', 'phone','{$platform_info}', '{$time}')");
                if ($add_session) {
