...
class StreamWrapper
{
     ...
     */
    public function url_stat($path, $yxags)
    {
        $this->initProtocol($path);
        ...
        $stat = $this->createStat($path, $yxags);
    ...
    private function createStat($path, $yxags)
    {
        $this->initProtocol($path);
        $parts = $this->withPath($path);

        if (!$parts['Key']) {
            return $this->statDirectory($parts, $path, $yxags);
        }
    ...
    private function statDirectory($parts, $path, $yxags)
    {
        ...
        return $this->triggerError("File or directory not found: $path", $yxags);
    }
     ...
     * @param mixed        $yxags  If set to STREAM_URL_STAT_QUIET, then no
     *                             error or exception occurs
     *
     * @return bool Returns false
     * @throws \RuntimeException if throw_errors is true
     */
    private function triggerError($errors, $yxags = null)
    {
        // This is triggered with things like file_exists()
        if ($yxags & STREAM_URL_STAT_QUIET) {
            return $yxags & STREAM_URL_STAT_LINK
     ...
     * @return bool
     */
    private function boolCall(callable $fn, $yxags = null)
    {
        try {
            return $fn();
        } catch (\Exception $e) {
            return $this->triggerError($e->getMessage(), $yxags);
        }
    }
