...
if (!empty($_GET['publish'])) {
    if ($_GET['publish'] == 'true') {
        ...
        $update = YX_UpdatePost($id, $form_data_array, 'list');
    }
}
$lists = $yx['lists'] = YX_GetPost($_GET['id'], $page, 'lists');
if (empty($lists)) {
    header("Location:" . YX_Link('404'));
    exit();
}
$yx['is_post_owner'] = YX_IsPostOwner($lists['id'], 'lists');
if ($lists['viewable'] == 0) {
...
$update_views = YX_UpdateViews('lists', $lists['id']);
...
$yx['title']       = $lists['title'];
$yx['description'] = $lists['description'];
$yx['page']        = 'lists';
$yx['keywords']    = $lists['tags'];
...
if ($yx['is_post_owner'] == true) {
	...
	if ($yx['lists']['active'] == 0 && $yx['lists']['viewable'] == 1) {
	...
	} else if ($yx['lists']['viewable'] == 0 && $yx['lists']['active'] == 0) {
...
$share_onclick = "YX_ShareLink(this.href, event,"  . $yx['lists']['id'] . ", 'lists');";
$share_twitter_onclick = "YX_ShareLink('none', event,"  . $yx['lists']['id'] . ", 'lists');";
$tags = '';
foreach (YX_GetPostTags($yx['lists']['tags']) as $key => $tag) {
    $tags .= '<a class="tags lists" href="{{LINK tags/' . $tag . '}}">' . $tag . '</a> ';
}
$yx['story']['is_active'] = 0;
if ($yx['lists']['active'] == 1) {
...
$comments = '';
foreach (YX_GetStoryComments(array('post_id' => $yx['lists']['id'],'page' => $yx['page'])) as $key => $yx['comment']) {
    $replies            = '';
...
$yx['user_reactions']  = YX_GetPercentageOfReactions($yx['lists']['id'],$yx['page']);
$user_reactions        = YX_LoadPage('story/user-reactions',array(
    'STORY_ID' => $yx['lists']['id'],
    'STORY_PAGE' => $yx['page']
));

$page_context  = array(
    'STORY_ID' => $yx['lists']['id'],
    'STORY_TITLE' => $yx['lists']['title'],
    'STORY_THUMB' => $yx['lists']['image'],
    'STORY_DESC' => $yx['lists']['description'],
    'STORY_ADMIN_OPTIONS' => $lists_admin_option,
    'STORY_VIEWS' => $yx['lists']['views'],
    'STORY_ENTRIES' => YX_ForEachEntries($yx['lists']['entries']),
    'STORY_AD' => YX_GetAd('between', false),
    'CATEGORY_NAME' => (!empty($yx['list_categories'][$yx['lists']['category']])) ? $yx['list_categories'][$yx['lists']['category']]: '',
    'CATEGORY_LINK' => '{{LINK latest-lists/' . $yx['lists']['category'] . '}}',
    'STORY_POSTED_TIME' => $yx['lists']['posted'],
    'STORY_LINK' => $yx['lists']['url'],
    'STORY_UPDATED_TIME' => $yx['lists']['updated'],
    'STORY_ENCODED_URL' => urlencode($yx['lists']['url']),
    'STORY_SHARES' => $yx['lists']['shares'],

    'NEXT_LINK' => $next_link,
    'BACK_LINK' => $back_link,
    
    'EDIT_BUTTON' => YX_Link('edit-post/' . $yx['lists']['id'] . '?type=lists'),
    'DELETE_BUTTON' => YX_Link('delete-post/' . $yx['lists']['id'] . '?type=lists'),
    'PUBISH_BUTTON' => $publish_button,
    
    'PUBLISHER_NAME' => $yx['lists']['publisher']['name'],
    'PUBLISHER_AVATAR' => $yx['lists']['publisher']['avatar'],
    'PUBLISHER_VERIFIED' => ($yx['lists']['publisher']['verified'] == 1) ? '<span class="verified-icon"><i class="fa fa-check-circle"></i></span>' : '',
    'PUBLISHER_URL' => $yx['lists']['publisher']['url'],
    'PUBLISHER_GENDER' => ($yx['lists']['publisher']['gender'] == 'male') ? $yx['lang']['male'] : $yx['lang']['female'],
    'PUBLISHER_COUNTRY' => (!empty($yx['lists']['publisher']['country_id'])) ? ', Lives in ' . $yx['countries_name'][$yx['lists']['publisher']['country_id']] : '',
    ...
    'STORY_TAGS' => $tags,
    'STORY_COMMENT_TOTAL' => YX_CountPostComments($yx['lists']['id'],$yx['page']),
