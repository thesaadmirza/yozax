...
if (!empty($_GET['publish'])) {
    if ($_GET['publish'] == 'true') {
        ...
        $update = YX_UpdatePost($id, $form_data_array, 'music');
    }
}
$music = $yx['music'] = YX_GetPost($_GET['id'], $page, 'music');
if (empty($music)) {
    header("Location:" . YX_Link('404'));
    exit();
}

$yx['is_post_owner'] = YX_IsPostOwner($music['id'], 'music');
if ($music['viewable'] == 0) {
...
$update_views = YX_UpdateViews('music', $music['id']);
...
$yx['title']       = $music['title'];
$yx['description'] = $music['description'];
$yx['page']        = 'music';
$yx['keywords']    = $music['tags'];
...
if ($yx['is_post_owner'] == true) {
	...
	if ($yx['music']['active'] == 0 && $yx['music']['viewable'] == 1) {
	...
	} else if ($yx['music']['viewable'] == 0 && $yx['music']['active'] == 0) {
...
$share_onclick = "YX_ShareLink(this.href, event,"  . $yx['music']['id'] . ", 'music');";
$share_twitter_onclick = "YX_ShareLink('none', event,"  . $yx['music']['id'] . ", 'music');";
$tags = '';
foreach (YX_GetPostTags($yx['music']['tags']) as $key => $tag) {
    $tags .= '<a class="tags music" href="{{LINK tags/' . $tag . '}}">' . $tag . '</a> ';
}
$yx['story']['is_active'] = 0;
if ($yx['music']['active'] == 1) {
...
$comments = '';
foreach (YX_GetStoryComments(array('post_id' => $yx['music']['id'],'page' => $yx['page'])) as $key => $yx['comment']) {
    $replies            = '';
...
$yx['user_reactions']  = YX_GetPercentageOfReactions($yx['music']['id'],$yx['page']);
$user_reactions        = YX_LoadPage('story/user-reactions',array(
    'STORY_ID' => $yx['music']['id'],
    'STORY_PAGE' => $yx['page']
));

$page_context  =  array(
    'STORY_ID' => $yx['music']['id'],
    'STORY_TITLE' => $yx['music']['title'],
    'STORY_THUMB' => $yx['music']['image'],
    'STORY_DESC' => $yx['music']['description'],
    'STORY_ADMIN_OPTIONS' => $lists_admin_option,
    'STORY_VIEWS' => $yx['music']['views'],
    'STORY_ENTRIES' => YX_ForEachEntries($yx['music']['entries']),
    'STORY_AD' => YX_GetAd('between', false),
    'CATEGORY_NAME' => (!empty($yx['music_categories'][$yx['music']['category']])) ? $yx['music_categories'][$yx['music']['category']]: '',
    'CATEGORY_LINK' => '{{LINK latest-music/' . $yx['music']['category'] . '}}',
    'STORY_POSTED_TIME' => $yx['music']['posted'],
    'STORY_UPDATED_TIME' => $yx['music']['updated'],
    'STORY_LINK' => $yx['music']['url'],
    'STORY_ENCODED_URL' => urlencode($yx['music']['url']),
    'STORY_SHARES' => $yx['music']['shares'],

    'NEXT_LINK' => $next_link,
    'BACK_LINK' => $back_link,
    
    'EDIT_BUTTON' => YX_Link('edit-post/' . $yx['music']['id'] . '?type=music'),
    'DELETE_BUTTON' => YX_Link('delete-post/' . $yx['music']['id'] . '?type=music'),
    'PUBISH_BUTTON' => $publish_button,
    
    'PUBLISHER_NAME' => $yx['music']['publisher']['name'],
    'PUBLISHER_AVATAR' => $yx['music']['publisher']['avatar'],
    'PUBLISHER_VERIFIED' => ($yx['music']['publisher']['verified'] == 1) ? '<span class="verified-icon"><i class="fa fa-check-circle"></i></span>' : '',
    'PUBLISHER_URL' => $yx['music']['publisher']['url'],
    'PUBLISHER_GENDER' => ($yx['music']['publisher']['gender'] == 'male') ? $yx['lang']['male'] : $yx['lang']['female'],
    'PUBLISHER_COUNTRY' => (!empty($yx['music']['publisher']['country_id'])) ? ', Lives in ' . $yx['countries_name'][$yx['music']['publisher']['country_id']] : '',
    ...
    'STORY_TAGS' => $tags,
    'STORY_COMMENT_TOTAL' => YX_CountPostComments($yx['music']['id'],$yx['page']),
